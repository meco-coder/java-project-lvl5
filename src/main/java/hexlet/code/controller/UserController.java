...
public class UserController {
    ...
    @PreAuthorize("""
                @userRepository.findById(#id).get().getEmail() == authentication.getName()
            """)
    public void deleteUser(@PathVariable final long id) throws UnprocessableEntityException {
        if (!userRepository.findById(id).get().getAuthorTasks().isEmpty() ||
                !userRepository.findById(id).get().getExecutorTasks().isEmpty()) {
            throw new UnprocessableEntityException("Пользователь имеет одну из задач, его нельзя удалить");
        }
